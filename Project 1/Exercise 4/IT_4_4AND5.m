...
[~, min_kl_index] = min(KL_divergences);
best_binomial_pmf = binomial_pmfs{min_kl_index};

%% Plot the binomial PMF with the minimum KL divergence
figure('Position', [0, 0, 1000, 600]);
bar(obs_data, best_binomial_pmf);
xlabel('Number of Successes');
ylabel('Probability');
title(['Binomial PMF with Minimum KL Divergence (p=', num2str(best_p, '%.4f'), ', KL Divergence=', num2str(min_KL_divergence, '%.4f'), ')']);
legend('Binomial PMF');
